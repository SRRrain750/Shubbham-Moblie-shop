...
const Home = () => {
  ...
  return (
    <section className='bg-white'>
      ...
      <div className='container mx-auto px-4 py-2 grid grid-cols-5 md:grid-cols-8 lg:grid-cols-10 gap-2 mt-3'>
        {loadingCategory ? (
          new Array(12).fill(null).map((c, index) => {
            return (
              <div key={index + "loadingCategory"} className='bg-white rounded p-4 min-h-36 grid gap-2 shadow animate-pulse'>
          ...
          categoryData.map((cat, index) => {
            return (
              ...
              >
                ...
                <p className='text-center text-sm font-medium mt-1'>{cat.name}</p>
              </div>
            )
          })
        )}

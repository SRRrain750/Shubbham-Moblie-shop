...
//       <div className='container top-24  mx-auto grid grid-cols-[90px,1fr]  md:grid-cols-[200px,1fr] lg:grid-cols-[280px,1fr]'>
//         {/**sub category **/}
    ...
    <section className="pt-24">
      ...
      <div className="container mx-auto flex gap-2">
          ...
          {DisplaySubCatory.map((s, index) => {
            ...
            return (
              <Link
                key={index}
                to={link}
                className={`block p-2 border-b cursor-pointer hover:bg-green-100 ${
                  subCategoryId === s._id ? "bg-green-100" : ""
                }`}
              >
                <img
                  src={s.image}
                  alt="subCategory"
                  className="w-12 h-12 mx-auto object-scale-down"
                />
                <p className="text-center text-xs md:text-sm mt-1">{s.name}</p>
              </Link>
            );
          })}
        </div>

        {/* ------------ RIGHT SIDE PRODUCTS (FULL VERTICAL SCROLL) ------------ */}
        <div className="flex-1">
          ...
          {/* PRODUCT GRID (This ensures vertical layout) */}
